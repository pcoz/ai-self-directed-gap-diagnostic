...
> **The key statistical insight:** with three examples you can estimate a mean, but not a variance. Marginalisation gives you the mean profile along an axis. Sharpening gives you fine structure that requires variance estimates to interpret. So: marginalise first, then compare.
...
| Approach | What it does | What it needs that this method doesn't |
|---|---|---|
| **Prototype Networks** (Snell et al., 2017) | Average then compare in embedding space | A **learned embedding** trained on thousands of prior tasks |
| **MAML / meta-learning** (Finn et al., 2017) | Learn to adapt quickly from few examples | A **distribution of similar tasks** to meta-train on |
| **AutoML / NAS** | Search over architectures using validation performance | **Hundreds of examples** to evaluate each candidate |
| **CAAFE** (Hollmann et al., 2023) | LLM proposes features from dataset descriptions | Doesn't **diagnose failure** or revise based on what went wrong |
| **The AI Scientist** (Sakana AI, 2024) | LLM-directed experimental loops | Operates at "try something else" — no **specific diagnosis** of which classes are confused and why |
| **Bayesian marginalisation** | Integrate out nuisance parameters | Applied to **model parameters**, not to spatial/temporal signal dimensions |
| **Dunn Index** | Min/max cluster separation metric | Evaluates a **completed clustering**, not an iterative search signal |
...
| Condition | Method works | Method loses advantage |
|---|---|---|
| **Labeled examples** | 2-10 per class | 100+ per class |
| **Noise** | High — marginalisation suppresses it | Low — sharpening works fine |
| **Observation size** | Variable — marginals are size-invariant | Uniform — raw comparison works |
| **Pretrained model** | None available | Available — transfer learning is better |
...
> **Iteration 1** tested temporal and spectral profiles on variable-length spectrograms. Temporal profiles **failed catastrophically** (36% accuracy) because short events produced degenerate spectrograms with as few as 2 time bins. The gap diagnostic correctly flagged this — temporal configurations had gaps of -0.92, while spectral configurations had gaps of -0.03.
>
> *Diagnosis: the temporal axis has inconsistent resolution across events of different duration.*

> **Iteration 2** attempted to fix the temporal problem by adding scalar temporal descriptors (duration, onset sharpness, decay rate). This **made accuracy worse** (41%), not better.
>
> *Diagnosis: 7 scalar features estimated from 3 seeds have too much variance — a single outlier seed shifts the class centroid enough to misclassify the majority. The fix needed more dimensions, not fewer.*

> **Iteration 3** replaced spectrogram-derived temporal features with **waveform envelope profiles** — energy distribution computed directly from the raw signal, which has consistent time resolution regardless of event duration. Combined with the spectral profile, this produced **66% accuracy at SNR 3 dB**, beating raw pixel comparison (62%) and far exceeding sharpening-based edge detection (29%).
